...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	 ...
	 */
	public Member findBySeller(Seller seller) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		...
		Predicate restrictions = criteriaBuilder.conjunction();
		if (seller != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("seller"), seller.getId()));
		}

		criteriaQuery.where(restrictions);
		TypedQuery<Member> query = entityManager.createQuery(criteriaQuery).setFlushMode(FlushModeType.COMMIT);
