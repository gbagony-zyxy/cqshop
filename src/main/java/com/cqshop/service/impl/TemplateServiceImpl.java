...
import com.cqshop.framework.Template;
import com.cqshop.framework.Template.Type;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/cqshop/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(cqshopXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/chshop/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
	...
	public Template get(String id) {
		try {
			File cqshopXmlFile = new ClassPathResource(CommonAttributes.CQSHOP_XML_PATH).getFile();
			Document document = new SAXReader().read(cqshopXmlFile);
			Element element = (Element) document.selectSingleNode("/cqshop/template[@id='" + id + "']");
			Template template = getTemplate(element);
			return template;
		} catch (Exception e) {
			e.printStackTrace();
