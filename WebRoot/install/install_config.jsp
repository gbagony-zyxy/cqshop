...
<%@page import="com.cqshop.framework.CommonAttributes"%>
<%@page import="com.cqshop.util.JsonUtils"%>
...
<%
	...
	if (status.equals("success")) {
		...
		try {
			Resource resource = new ClassPathResource(CommonAttributes.CQSHOP_PROPERTIES_PATH);
			...
			status = "error";
			message = "cqshop.PROPERTIES文件写入失败!";
			exception = stackToString(e);
		}
	}
	...
	if (status.equals("success")) {
		try {
			...
			Element siteUrlElement = (Element) document.selectSingleNode("/cqshop/setting[@name='siteUrl']");
			Element logoElement = (Element) document.selectSingleNode("/cqshop/setting[@name='logo']");
			Element defaultLargeProductImageElement = (Element) document.selectSingleNode("/cqshop/setting[@name='defaultLargeProductImage']");
			Element defaultMediumProductImageElement = (Element) document.selectSingleNode("/cqshop/setting[@name='defaultMediumProductImage']");
			Element defaultThumbnailProductImageElement = (Element) document.selectSingleNode("/cqshop/setting[@name='defaultThumbnailProductImage']");
			String siteUrl;
			...
			try {
				...
				fileOutputStream = new FileOutputStream(cqshopXmlFile);
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_LOCK_CONFIG_PATH), "SHOP++ INSTALL LOCK - cqshop.NET");
		} catch (IOException e) {
			status = "error";
	...
	if (status.equals("success")) {
		try {
			FileUtils.writeStringToFile(new File(rootPath + INSTALL_INIT_CONFIG_PATH), "SHOP++ INSTALL INIT - cqshop.NET");
